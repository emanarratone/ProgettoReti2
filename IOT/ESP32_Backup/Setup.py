...
try:
    res_veicolo = gestore.richiedi("veicolo/richiesta", payload_veicolo, topic_risposta_auto)
    dati_veicolo = pulisci_dati(res_veicolo)

    if dati_veicolo:
        
        tipo = dati_veicolo.get('tipoVeicolo')
        print(f"✅ RISPOSTA RICEVUTA: Veicolo {targa_veicolo} è di Classe {tipo}")
    else:
        print(f"⚠️ ATTENZIONE: Risposta vuota dal server per la targa {targa_veicolo}")

except Exception as e:
    print(f"❌ ERRORE CRITICO: Timeout o problema di comunicazione: {e}")
